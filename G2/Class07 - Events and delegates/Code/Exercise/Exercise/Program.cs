...
namespace Exercise
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            List<string> orderedPeople = people.Where(x => x.FirstName.StartsWith("R"))
            ...
            List<string> orderedDogs = dogs.Where(x => x.Color == "Brown" && x.Age > 3)
                                        .OrderBy(x => x.Age)
                                        .Select(x => $"{x.Name} {x.Age}")
                                        .ToList();

            List<Person> peopleWithMoreThan2Dogs = people.Where(x => x.Dogs.Count > 2)
                                                    .OrderByDescending(x => x.FirstName)
                                                    .ToList();
            List<string> freddysDogs = people.FirstOrDefault(x => x.FirstName == "Freddy")?
                                        .Dogs.Where(x => x.Age > 1).Select(x => x.Name).ToList();

            Dog dog = people.FirstOrDefault(x => x.FirstName == "Nathen")?.Dogs.First();

            List<string> dogNames = people.Where(x => x.FirstName == "Cristofer" || x.FirstName == "Freddy"
                          || x.FirstName == "Erin" || x.FirstName == "Amelia").SelectMany(x => x.Dogs)
